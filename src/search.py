...
def search_string_func(normal_text, text, active_settings):  # 
    ...
    if artist:
        docs_array, scores_array = luc_engine.search_index("lyrics", "artist", normal_text, False, top_k)
        lyr_docs_array.extend(docs_array)
        lyr_scores_array.extend(scores_array)

    if song_name:
        docs_array, scores_array = luc_engine.search_index("lyrics", "song_name", normal_text, False, top_k)
        lyr_docs_array.extend(docs_array)
        lyr_scores_array.extend(scores_array)
        
    if lyrics:
        docs_array, scores_array = luc_engine.search_index("lyrics", "lyrics", text, False, top_k)
        lyr_docs_array.extend(docs_array)
        lyr_scores_array.extend(scores_array)
...
def search_artist_func(artist):
    ...
    docs_array, scores_array = luc_engine.search_index("albums", "artist", artist, False, 50)
