...
def search_index(folder, column, query_string, case, max_results=500):  # to search in the modified folder  
    global already
    if (case):
        already.append(query_string.lower())
    ...
    for hit in hits.scoreDocs:
        # print(hit)
        if (query_string.lower() in already):
            docs_array.append(hit.doc + 1)
            print(f"Score: {str(hit.score)[:5]}, doc_id: {hit.doc + 1}")
        else:
